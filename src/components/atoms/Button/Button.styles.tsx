...
export const Wrapper = styled.div<ButtonStyledProps>`
	...
	.MuiButton-root {
		background: ${({ color }) =>
			isUndefined(color)
				? selectiveColors.blue.normal
				: selectiveColors[asNonUndefined(color)].normal};
		text-transform: none;
		...
		&:hover {
			background: ${({ color }) =>
				isUndefined(color)
					? selectiveColors.blue.dark
					: selectiveColors[asNonUndefined(color)].dark};
		}

		&:focus {
			background: ${({ color }) =>
				isUndefined(color)
					? selectiveColors.blue.light
					: selectiveColors[asNonUndefined(color)].light};
		}
